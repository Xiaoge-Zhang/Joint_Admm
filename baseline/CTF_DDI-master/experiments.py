...
class Experiments(object):
    ...
    def CV_triplet(self):
        ...
        for k in range(k_folds):
            ...
            elif self.model_name == 'TFAI_CP_within_mod':
                predict_tensor = self.model()(train_tensor, S1, S2,
                                          r=self.parameters['r'],
                                          alpha=self.parameters['alpha'], beta=self.parameters['beta'],
                                          lam=self.parameters['lam'],
                                          tol=self.parameters['tol'], max_iter=self.parameters['max_iter'])
            elif self.model_name == 'TDRC':
                predict_tensor = self.model()(train_tensor, S1, S2,
                                          r=self.parameters['r'],
                                          alpha=self.parameters['alpha'], beta=self.parameters['beta'],
                                          lam=self.parameters['lam'],
                                          tol=self.parameters['tol'], max_iter=self.parameters['max_iter'])
            elif self.model_name == 'CP':
                predict_tensor = self.model()(train_tensor, S1, S2,
                                          r=self.parameters['r'],
                                          tol=self.parameters['tol'], max_iter=self.parameters['max_iter'])
            else:
...
def parse_arguments():
    ...
    parser.add_argument("--model", type=str, default="CTF", help='model to be used')
...
if __name__ == '__main__':
    ...
    if data_source == 'x' or data_source == 'y':

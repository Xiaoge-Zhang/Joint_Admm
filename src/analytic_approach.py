...
def create_scale_matrix(matrix):
    """
    Create a diagonal scale matrix where each diagonal element is the sum of a column
    from the input 2D NumPy array.

    Args:
        matrix (np.ndarray): Input array of shape (n, m)

    Returns:
        np.ndarray: Diagonal matrix of shape (m, m)
    """
    if matrix.ndim != 2:
...
def khatri_rao(A, B):
    """
    Compute the Khatri–Rao product of two matrices A and B.
    A ∈ ℝ^{I×K}, B ∈ ℝ^{J×K}
    Result ∈ ℝ^{(I*J)×K}

    Args:
        A (np.ndarray): Matrix of shape (I, K)
        B (np.ndarray): Matrix of shape (J, K)

    Returns:
        np.ndarray: Khatri–Rao product of shape (I * J, K)
    """
...
def resemble_tensor(A, B, C):
    """
    Reconstructs a 3D tensor from factor matrices A, B, C using CP decomposition.

    Args:
        A: np.ndarray of shape (I, R)
        B: np.ndarray of shape (J, R)
        C: np.ndarray of shape (K, R)

    Returns:
        X_hat: np.ndarray of shape (I, J, K)
    """
    I, R = A.shape
    J = B.shape[0]
    K = C.shape[0]
...
def generate_test_tensor(tensor, args):
    """
    Randomly selects test indices (positive and negative) from a symmetric 3D tensor.

    Args:
        tensor (np.ndarray): 3D tensor (num_drug, num_drug, num_disease), assumed symmetric on first two axes.
        test_ratio (float): Fraction of 1s to hold out for testing.
        test_rnd_seed (int): Random seed.
        missing_rate (float): Fraction of remaining 1s to mask as missing (set to 0).

    Returns:
        modified_tensor (np.ndarray): Copy of input tensor with test indices zeroed.
        all_test_indices (np.ndarray): Array of shape (2 * num_test, 3) with test indices and their symmetric counterparts.
    """

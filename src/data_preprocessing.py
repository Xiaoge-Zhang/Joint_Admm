...
if __name__ == '__main__':
    ...
    if not non_binary:
        ...
        with open(f'../baseline/CTF_DDI-master/ADMM/{x_file_name}_{y_file_name}_x.pickle', 'wb') as handle:
            pickle.dump(x_tensor, handle, protocol=4)
    else:
        with open(f'../data/{x_file_name}_{y_file_name}_x.pickle', 'wb') as handle:
            pickle.dump(x_tensor, handle, protocol=4)
        with open(f'../baseline/CTF_DDI-master/ADMM/{x_file_name}_{y_file_name}_z.pickle', 'wb') as handle:
            pickle.dump(x_tensor, handle, protocol=4)
    ...
    '''
    # OPTINAL: filter out the positive DDI types
    filter_df = pd.read_csv('../data/DDI/mapping_with_property_manual.csv')
    # Identify numbers with "positive" property in the DataFrame
    positive_numbers = set(filter_df.loc[filter_df["property"] == "positive", "mapping_index"])
    # Filter the list to exclude numbers with "positive" property
    filtered_numbers = [num for num in ddi_types if num not in positive_numbers]
    ddi_types = filtered_numbers
    '''
    ...
    with open(f'../baseline/CTF_DDI-master/ADMM/{x_file_name}_{y_file_name}_y.pickle', 'wb') as handle:
        pickle.dump(y_tensor, handle, protocol=4)
